...
local watchedSpells = addon.WatchedSpells
local runningDR = {}
...
local function ParseCLEU(self, _, timestamp, event, ...)	
	...
	if increase and watchedSpells[spell] then 
		local targetDR = runningDR[guid]
		...
		for category, spells in pairs(categories) do
			if spells[spell] then
				if category == '*' then category = spell end
				local dr = targetDR[category]
				local now = GetTime()
				if not dr then
					dr = new()
					dr.name = name
					dr.texture = select(3, GetSpellInfo(spellId))
					dr.count = 0
